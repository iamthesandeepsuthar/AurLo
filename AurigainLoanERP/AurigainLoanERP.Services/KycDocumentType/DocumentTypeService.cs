...
namespace AurigainLoanERP.Services.KycDocumentType
{
    public class DocumentTypeService : BaseService, IDocumentTypeService
    {
        ...
        public async Task<ApiServiceResponseModel<List<DocumentTypeModel>>> GetAllAsync(IndexModel model)
        {
            ...
            try
            {
                var result = (from role in _db.DocumentTypes
        ...
        public async Task<ApiServiceResponseModel<List<DDLDocumentTypeModel>>> GetDocumentTypes()
        {
            try
            {
                var types = await _db.DocumentTypes.Select(x => new DDLDocumentTypeModel
                {
                    Id = x.Id,
                    Name = x.DocumentName
                }).ToListAsync();
        ...
        public async Task<ApiServiceResponseModel<DocumentTypeModel>> GetById(int id)
        {
            ...
            try
            {
                var result = await (from c1 in _db.DocumentTypes
        ...
        public async Task<ApiServiceResponseModel<string>> AddUpdateAsync(DocumentTypeModel model)
        {
            try
            {
                if (model.Id == 0)
                {
                    ...
                    var result = await _db.DocumentTypes.AddAsync(type);
                }
                else
                {
                    var type = await _db.DocumentTypes.FirstOrDefaultAsync(x => x.Id == model.Id);
        ...
        public async Task<ApiServiceResponseModel<object>> UpdateDeleteStatus(int id)
        {
            try
            {
                DocumentType objRole = await _db.DocumentTypes.FirstOrDefaultAsync(r => r.Id == id);
        ...
        public async Task<ApiServiceResponseModel<object>> UpateActiveStatus(int id)
        {
            try
            {
                DocumentType type = await _db.DocumentTypes.FirstOrDefaultAsync(r => r.Id == id);
