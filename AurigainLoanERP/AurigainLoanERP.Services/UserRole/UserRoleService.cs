...
namespace AurigainLoanERP.Services.UserRoles
{
    public class UserRoleService : BaseService, IUserRoleService
    {
        ...
        public async Task<ApiServiceResponseModel<string>> AddUpdateAsync(UserRolePostModel model)
        {
            ...
            try
            {
                ...
                if (model.Id == 0)
                {
                    ...
                    var result = await _db.UserRoles.AddAsync(userRole);

                }
                else
                {
                    var userRole = await _db.UserRoles.FirstOrDefaultAsync(x => x.Id == model.Id);
        ...
        public async Task<ApiServiceResponseModel<object>> CheckRoleExist(string name, int? id = null)
        {
            try
            {
                UserRole objRole = await _db.UserRoles.FirstOrDefaultAsync(r => !r.IsDelete && name.ToLower().Equals(r.Name.ToLower()) && (id == null || id == 0 || r.Id != id));
