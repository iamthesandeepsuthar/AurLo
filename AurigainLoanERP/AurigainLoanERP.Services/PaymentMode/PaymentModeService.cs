   ...
   public class PaymentModeService:BaseService, IPaymentModeService
    {
        ...
        public async Task<ApiServiceResponseModel<List<PaymentModeModel>>> GetAllAsync(IndexModel model)
        {
            ...
            try
            {
                var result = (from role in _db.PaymentModes
                              where !role.IsDelete && (string.IsNullOrEmpty(model.Search) || role.Mode.Contains(model.Search))
                              orderby (model.OrderByAsc == 1 && model.OrderBy == "Name" ? role.Mode : "") ascending
                              orderby (model.OrderByAsc != 1 && model.OrderBy == "Name" ? role.Mode : "") descending
                              select role);
        ...
        public async Task<ApiServiceResponseModel<List<DDLPaymentModeModel>>> GetPaymentModes()
        {
            try
            {
                var modes = await _db.PaymentModes.Select(x => new DDLPaymentModeModel
                {
                    Id = x.Id,
                    Mode = x.Mode
                }).ToListAsync();
        ...
        public async Task<ApiServiceResponseModel<PaymentModeModel>> GetById(int id)
        {
            ...
            try
            {
                var result = await (from c1 in _db.PaymentModes
        ...
        public async Task<ApiServiceResponseModel<string>> AddUpdateAsync(PaymentModeModel model)
        {
            try
            {
                if (model.Id == 0)
                {
                    ...
                    var result = await _db.PaymentModes.AddAsync(mode);
                }
                else
                {
                    var mode = await _db.PaymentModes.FirstOrDefaultAsync(x => x.Id == model.Id);
        ...
        public async Task<ApiServiceResponseModel<object>> UpdateDeleteStatus(int id)
        {
            try
            {
               AurigainLoanERP.Data.Database.PaymentMode objRole = await _db.PaymentModes.FirstOrDefaultAsync(r => r.Id == id);
        ...
        public async Task<ApiServiceResponseModel<object>> UpateActiveStatus(int id)
        {
            try
            {
                AurigainLoanERP.Data.Database.PaymentMode mode = await _db.PaymentModes.FirstOrDefaultAsync(r => r.Id == id);
