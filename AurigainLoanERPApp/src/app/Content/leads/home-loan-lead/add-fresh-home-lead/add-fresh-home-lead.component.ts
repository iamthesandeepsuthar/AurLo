...
@Component({
  selector: 'app-add-fresh-home-lead',
  templateUrl: './add-fresh-home-lead.component.html',
  styleUrls: ['./add-fresh-home-lead.component.scss'],
  providers: [UserSettingService,PersonalHomeCarLoanService,ProductService,StateDistrictService]
})
export class AddFreshHomeLeadComponent implements OnInit {
  ...
  getDDLProducts() {
    let serve = this._productService.GetProductbyCategory(ProductCategoryEnum.HomeLoan).subscribe(res => {
      serve.unsubscribe();
      if (res.IsSuccess) {
