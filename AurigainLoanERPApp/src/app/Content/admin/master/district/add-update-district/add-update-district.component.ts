...
export class AddUpdateDistrictComponent implements OnInit {
  ...
  constructor(private readonly fb: FormBuilder, private readonly _districtService: StateDistrictService,
    ...
    if (this.districtForm.valid) {
      let subscription = this._districtService.AddUpdateDistrict(this.model).subscribe(response => {
        ...
        if(response.IsSuccess) {
         ...
         this._router.navigate([this.routing_Url.AdminModule+'/'+this.routing_Url.MasterModule + this.routing_Url.District_List_Url]);
        } else {
